 ...
 */
export default function StackedAreaChart({ data, label = 'Composition du patrimoine' }) {
  const points = data ?? [];
  ...
  const totals = points.map((p) => types.reduce((s, t) => s + Number(p[t] ?? 0), 0));
  ...
  const bands = types.map((t) => {
    ...
    const bottom = lower.map((pt) => `${pt.x} ${pt.y}`).reverse().join(' L ');
  ...
  return (
    <div className="fchart">
      <svg className="fchart__svg" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet" role="img" aria-label={label}>
        {ticks.map((v, i) => (
          <g key={i}>
            ...
            <text className="fchart__ylabel" x={PAD.left - 8} y={yAt(v) + 4}>{formatEur(Math.round(v))}</text>
          </g>
        ))}

        {bands.map((b) => (
          <path key={b.type} d={b.d} fill={b.color} fillOpacity="0.55" stroke={b.color} strokeWidth="1" />
        ))}
      ...
      <ul className="fdonut__legend fchart__legend">
        {types.map((t) => (
          <li key={t}>
            <span className="fdonut__swatch" style={{ background: TYPE_META[t].color }} />
            <span className="fdonut__legendlabel">{TYPE_META[t].label}</span>
            <span className="fdonut__legendpct">{formatEur(Number(points[n - 1][t] ?? 0))}</span>
          </li>
        ))}
      </ul>
    </div>

...
import { formatDuration, formatQuantity, isSection, scaleQuantity } from './constants';
...
export default function KnowHowDrawer({
  ...
  const hasQuantified = recipe.components.some((c) => c.quantity != null && !isSection(c));
  ...
  const scaledComponents = useMemo(
    () => recipe.components.map((c) => ({ ...c, scaledQty: scaleQuantity(c.quantity, scale) })),
    ...
    <div className="rdrawer-overlay" onMouseDown={onClose}>
      <aside className="rdrawer" onMouseDown={(e) => e.stopPropagation()}>
        <header className="rdrawer__head">
          <button className="rdrawer__close" onClick={onClose} aria-label="Fermer">←</button>
          <div className="rdrawer__heading">
            ...
            <div className="rdrawer__sub">
              <span className="rdrawer__cat" style={{ '--c': category.color }}>{category.name}</span>
              {recipe.labels.map((l) => (
            ...
            {(recipe.yieldText || recipe.totalTimeMin != null) && (
              <p className="rdrawer__meta">
                {[recipe.yieldText, recipe.totalTimeMin != null ? formatDuration(recipe.totalTimeMin) : null]
                  .filter(Boolean)
                  .join(' · ')}
              </p>
            )}
        ...
        <div className="rdrawer__actions">
          <button className="btn btn--primary" onClick={() => onRealize(recipe, scale)}>▶ Réaliser</button>
        ...
        {recipe.goal && (
          <p className="rdrawer__goal"><strong>Objectif : </strong>{recipe.goal}</p>
        )}
          ...
          <section className="rdrawer__section">
            <div className="rdrawer__sectionhead">
              ...
              {hasQuantified && (
                <div className="rscale">
                  <span className="rscale__label">{hasBase ? 'Portions' : 'Quantité'}</span>
                  <button className="rscale__btn" onClick={() => (hasBase ? stepPortions(-1) : stepFactor(-0.5))} aria-label="Diminuer">−</button>
                  <span className="rscale__value">
                    {hasBase ? formatQuantity(portions) : `×${formatQuantity(factor)}`}
                  </span>
                  <button className="rscale__btn" onClick={() => (hasBase ? stepPortions(1) : stepFactor(0.5))} aria-label="Augmenter">+</button>
                </div>
              )}
            </div>
            <ul className="rdrawer__components">
              {scaledComponents.map((c) =>
                isSection(c) ? (
                  <li key={c.id} className="rdrawer__compsection">{c.label.replace(/^[—-]\s*|\s*[—-]$/g, '')}</li>
                ) : (
                  <li key={c.id} className="rdrawer__comp">
                    {c.scaledQty != null && (
                      <span className="rdrawer__compqty">
                        {formatQuantity(c.scaledQty)}{c.unit ? ` ${c.unit}` : ''}
                      </span>
                    )}
                    <span className="rdrawer__complabel">
                      {c.label}
                      {c.note && <span className="rdrawer__compnote"> — {c.note}</span>}
                    </span>
                  </li>
                ),
              )}
        ...
        {recipe.components.length === 0 && recipe.steps.length === 0 && !recipe.goal && (
          <p className="rdrawer__empty">Ce savoir-faire n'a pas encore de contenu. Clique sur « Éditer » pour l'enrichir.</p>
        )}
      </aside>
    </div>
  );

      ...
      <div className="boverview">
        <section className="bcardbox">
          ...
          <ul className="bcounts">
            <li className="bcount"><span className="bcount__n t-up">{s.wonCount}</span><span className="bcount__l">Gagnants</span></li>
            <li className="bcount"><span className="bcount__n t-down">{s.lostCount}</span><span className="bcount__l">Perdants</span></li>
            <li className="bcount"><span className="bcount__n">{s.voidCount}</span><span className="bcount__l">Remboursés</span></li>
            <li className="bcount"><span className="bcount__n">{s.pendingCount}</span><span className="bcount__l">En cours</span></li>
          </ul>
          <div className="boverview__lines">
            <div><span>Mises jouées</span><strong>{formatEur(s.turnover)}</strong></div>
            <div><span>Mises en cours</span><strong>{formatEur(s.pendingStake)}</strong></div>
            <div><span>Capital de départ</span><strong>{formatEur(s.startingCapital)}</strong></div>
          </div>
        </section>
          ...
          <ul className="bstatlist">
            {highlights.map((h) => (
              <li key={h.label} className="bstat">
                ...
                <span className={`bstat__value${h.tone ? ` t-${h.tone}` : ''}`}>{h.value}</span>
              </li>
            ))}
            <li className="bstat">
            ...
            <li className="bstat">
              ...
              <span className={`bstat__value t-${trendClass(s.biggestWin)}`}>{formatSignedEur(s.biggestWin)}</span>
            </li>
          </ul>
        </section>
      </div>

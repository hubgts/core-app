...
export default function CapitalChart({ data, color = '#818cf8' }) {
  const points = useMemo(
    () => (data ?? []).map((d) => ({ date: d.date, value: Number(d.capital ?? 0) })),
    [data],
  );
  ...
  if (n < 2) {
    return (
      <div className="bchart">
        <svg className="bchart__svg" viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Courbe du capital">
  ...
  return (
    <div className="bchart">
      <svg className="bchart__svg" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet" role="img" aria-label="Courbe du capital">
        <defs>
        ...
        {ticks.map((v, i) => (
          <g key={i}>
            <line className="bchart__gridline" x1={PAD.left} x2={W - PAD.right} y1={yAt(v)} y2={yAt(v)} />
        ...
        {linePts.map((p, i) => (
          <circle key={i} className="bchart__dot" cx={p.x} cy={p.y} r={2.5} fill={color}>
        ...
        {points.map((p, i) =>
          i % labelEvery === 0 ? (
            <text key={i} className="bchart__xlabel" x={xAt(i)} y={H - PAD.bottom + 18}>
              {shortDate(p.date)}
            </text>
          ) : null,
        )}

...
export default function ProgramEditor({ program, onSaved, onCancel }) {
  ...
  const addPhase = () =>
    patch({
      phases: [
        ...draft.phases,
        { name: `Phase ${draft.phases.length + 1}`, objective: '' },
      ],
    });
  const patchPhase = (pi, p) =>
    patch({
      phases: draft.phases.map((ph, i) => (i === pi ? { ...ph, ...p } : ph)),
    });
  const removePhase = (pi) =>
    patch({
      ...
      weeks: draft.weeks.map((w) => {
        ...
        if (w.phaseIndex != null && w.phaseIndex > pi)
          return { ...w, phaseIndex: w.phaseIndex - 1 };
        return w;
      }),
    });
  ...
  const removeWeek = (wi) =>
    patch({ weeks: draft.weeks.filter((_, i) => i !== wi) });
  ...
  function deleteSession() {
    ...
    setDraft((d) => ({
      ...
      weeks: d.weeks.map((w, i) =>
        i === weekIdx
          ? { ...w, sessions: w.sessions.filter((_, j) => j !== sessionIdx) }
          : w,
      ),
    }));
    setSessionModal(null);
  ...
  async function save() {
    ...
    const payload = {
      ...
      phases: draft.phases.map((p) => ({
        name: p.name.trim(),
        objective: p.objective.trim() || null,
      })),
      weeks: draft.weeks.map((w) => ({
        index: w.index,
    ...
    <div className="prog-editor">
      ...
      <section className="prog-section">
        <div className="prog-section__head">
          ...
          <button className="btn btn--ghost btn--sm" onClick={addPhase}>
            + Phase
          </button>
        </div>
        {draft.phases.length === 0 ? (
          <p className="muted">
            Aucune phase. Les semaines peuvent exister sans phase.
          </p>
        ) : (
            ...
            {draft.phases.map((ph, pi) => (
              <div key={pi} className="prog-phase">
                ...
                <input
                  ...
                  onChange={(e) =>
                    patchPhase(pi, { objective: e.target.value })
                  }
                />
                <button
                  className="icon-btn"
      ...
      <section className="prog-section">
        ...
        {draft.weeks.map((w, wi) => (
          <div
            key={wi}
            className={`prog-week${w.isDeload ? ' is-deload' : ''}`}
          >
            <div className="prog-week__head">
              <span className="prog-week__index">
                S{w.index}
                {w.isDeload ? ' 🌙' : ''}
              </span>
              <Combobox
                ...
                value={w.phaseIndex ?? ''}
                onChange={(v) =>
                  patchWeek(wi, { phaseIndex: v === '' ? null : v })
                }
                options={[
              ...
              <label className="switch prog-week__deload">
                <input
                  ...
                  checked={w.isDeload}
                  onChange={(e) =>
                    patchWeek(wi, { isDeload: e.target.checked })
                  }
                />
                <span>Deload</span>
              </label>
              ...
              {PROGRAM_DAYS.map((d) => {
                ...
                return (
                  <div key={d.value} className="prog-day">
                    <div className="prog-day__label">
                      {d.code}
                      <span className="prog-day__sub">{d.label}</span>
                    </div>
                    {daySessions.map(({ s, idx }) => (
                      <button
                        ...
                        onClick={() =>
                          setSessionModal({ weekIdx: wi, sessionIdx: idx })
                        }
                      >
                        <span>{TYPE_META[s.type]?.icon}</span>
                        <span className="prog-pill__txt">
                    ...
                    <button
                      ...
                      onClick={() =>
                        setSessionModal({ weekIdx: wi, presetDay: d.value })
                      }
        ...
        <ProgramSessionModal
          session={
            sessionModal.sessionIdx != null
              ? draft.weeks[sessionModal.weekIdx].sessions[
                  sessionModal.sessionIdx
                ]
              : undefined
          }
